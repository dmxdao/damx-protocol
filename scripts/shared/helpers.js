...
const signers = {
  arbitrum: new ethers.Wallet(FTM_DEPLOY_KEY).connect(providers.arbitrum),
  avax: new ethers.Wallet(FTM_DEPLOY_KEY).connect(providers.avax)
}
...
async function sendTxn(txnPromise, label) {
  for (let i = 0; i < 5; i++) {
    try {
      ...
      console.log(err);
      if (i + 1 < 5) {
        console.log('Failed. Retrying will be started after a few seconds.')
        await sleep(2000)
      }
    }
  }
...
async function deployContract(name, args, label, options) {
  for (let i = 0; i < 5; i++) {
    try {
      ...
      console.log(err);
      if (i + 1 < 5) {
        console.log('Deploy failed. Retrying will be started after a few seconds.')
        await sleep(2000)
      }
    }
  }
